...
public class SystemConfigService : ISystemConfigService
{
    ...
    public async Task<IEnumerable<SystemConfigDto>> GetAllAsync()
    {
        ...
        return configs.OrderBy(c => c.ConfigKey).Select(MapToDto);
    }
    ...
    public async Task<SystemConfigDto> UpsertAsync(string key, UpsertSystemConfigDto dto, int? updatedByUserId)
    {
        ...
        var config = await unitOfWork.SystemConfigs.GetByKeyAsync(normalizedKey);
        ...
        if (config == null)
        {
            ...
            config = await unitOfWork.SystemConfigs.GetByIdAsync(config.Id);
