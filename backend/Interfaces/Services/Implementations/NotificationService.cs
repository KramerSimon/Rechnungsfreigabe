...
public class NotificationService : INotificationService
{
    ...
    public async Task EnsureApprovalNotificationForApproverAsync(int invoiceId, int approverId)
    {
        try
        {
            var allNotifs = await unitOfWork.Notifications.GetAllAsync();
            var exists = allNotifs.Any(n =>
            ...
            await CreateNotificationAsync(
                ...
                $"Rechnung {invoice.InvoiceNumber} von {invoice.Supplier.Name} �ber {invoice.TotalAmount:C} EUR wartet auf Ihre Freigabe.",
                invoiceId,
    ...
    {
        var notifications = await unitOfWork.Notifications.GetUserNotificationsAsync(userId, unreadOnly);
        ...
        int? invoiceId = null, NotificationPriority priority = NotificationPriority.Normal)
    {
        try
        {
            ...
            var createdNotification = await unitOfWork.Notifications.GetByIdWithIncludesAsync(notification.Id);
    ...
    public async Task<bool> MarkNotificationAsReadAsync(int notificationId, int userId)
    {
        try
        {
            var notification = await unitOfWork.Notifications.FirstOrDefaultAsync(
        ...
        try
        {
            ...
            foreach (var accountingUser in accountingUsers)
            {
                ...
                await CreateNotificationAsync(
                    ...
                    $"Rechnung {invoice.InvoiceNumber} von {invoice.Supplier.Name} �ber {invoice.TotalAmount:C} EUR ist eingegangen.",
                    invoiceId,
    ...
    public async Task NotifyInvoiceApprovalAsync(int invoiceId, bool approved)
    {
        try
        {
            ...
            var message = $"Rechnung {invoice.InvoiceNumber} von {invoice.Supplier.Name} �ber {invoice.TotalAmount:C} EUR wurde {actionText}.";
            ...
            // Notify accounting team
            var allUsers3 = await unitOfWork.Users.GetAllAsync();
            var accountingUsers = allUsers3
                .Where(u => u.IsActive && u.UserRoles.Any(ur => ur.Role.Name == "Buchhaltung"))
                .ToList();
    ...
    public async Task NotifyOverdueInvoicesAsync()
    {
        try
        {
            var bezahltStatus = await unitOfWork.Statuses.GetByCodeAndTypeAsync(
                RechnungsfreigabeAPI.Models.StatusCodes.Invoice.Bezahlt,
                EntityTypes.Invoice);
            var storniert = await unitOfWork.Statuses.GetByCodeAndTypeAsync(
                RechnungsfreigabeAPI.Models.StatusCodes.Invoice.Storniert,
                EntityTypes.Invoice);
            var ueberfaellig = await unitOfWork.Statuses.GetByCodeAndTypeAsync(
                RechnungsfreigabeAPI.Models.StatusCodes.Invoice.Ueberfaellig,
                EntityTypes.Invoice);
            ...
            foreach (var user in financeUsers)
            {
                if (overdueInvoices.Any())
                {
                    await CreateNotificationAsync(
                        user.Id,
                        "overdue_invoices",
                        "�berf�llige Rechnungen",
                        $"{overdueInvoices.Count} Rechnung(en) sind �berf�llig und ben�tigen Ihre Aufmerksamkeit.",
                        null,
