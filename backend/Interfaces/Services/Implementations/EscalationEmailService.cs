...
public class EscalationEmailService : IEscalationEmailService
{
    ...
    public async Task<bool> SendEscalationEmailAsync(int invoiceId, int escalationRuleId)
    {
        try
        {
            ...
            var rule = await unitOfWork.EscalationRules.GetByIdWithIncludesAsync(escalationRuleId);
    ...
    public async Task ProcessEscalationEmailsAsync()
    {
        try
        {
            var activeRules = await unitOfWork.EscalationRules.GetActiveRulesAsync();

            if (!activeRules.Any())
                return;

            // Get all invoices with status matching escalation triggers
            var inPruefung = await unitOfWork.Statuses.GetByCodeAndTypeAsync(
                RechnungsfreigabeAPI.Models.StatusCodes.Invoice.InPruefung,
                EntityTypes.Invoice);
            var freigabeErforderlich2 = await unitOfWork.Statuses.GetByCodeAndTypeAsync(
            ...
            var invoicesToCheck = allInvoices
            ...
            foreach (var invoice in invoicesToCheck)
            {
                foreach (var rule in activeRules)
                {
                    ...
                    if (ShouldEscalate(invoice, rule))
                    {
                        ...
                        var allEscalationLogs = await unitOfWork.EscalationLogs.GetAllAsync();
                        var alreadyEscalated = allEscalationLogs.Any(el =>
    ...
    public async Task EnsureEscalationTrackingAsync(int invoiceId, int escalationRuleId)
    {
        ...
        {
            var invoice = await unitOfWork.Invoices.GetByIdAsync(invoiceId);
            var rule = await unitOfWork.EscalationRules.GetByIdAsync(escalationRuleId);
            ...
            var allLogs = await unitOfWork.EscalationLogs.GetAllAsync();
            var existingLog = allLogs
                .FirstOrDefault(el =>
            ...
            if (existingLog == null)
            {
                ...
                unitOfWork.EscalationLogs.Add(log);
    ...
    private async Task<List<User>> GetRecipientEmailsAsync(EscalationRule rule)
    {
        ...
        if (rule.NotifyRoles?.Any() == true)
        {
            ...
            var allUsers2 = await unitOfWork.Users.GetAllAsync();
            var roleUsers = allUsers2
                .Where(u => u.IsActive && 
