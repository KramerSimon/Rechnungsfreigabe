...
public class InvoiceHistoryService : IInvoiceHistoryService
{
    ...
    {
        var histories = await unitOfWork.InvoiceHistories.GetByInvoiceIdAsync(invoiceId);
        return mapper.Map<List<InvoiceHistoryDto>>(histories.ToList());
    }
    ...
    public async Task CreateEscalationAsync(int invoiceId, string reason, int? escalatedTo = null)
    {
        ...
        var invoice = await unitOfWork.Invoices.GetForEscalationAsync(invoiceId);

        var targetUser = escalatedTo.HasValue
            ? await unitOfWork.Users.GetByIdAsync(escalatedTo.Value)
